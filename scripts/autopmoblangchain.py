...
# =====================
def run_extractor(
    passages: List[Dict[str, Any]], meta: Dict[str, Any]
) -> List[EquationItem]:
    """
...
def extractor_cli():
    ...
    from src_1.classify_extract_vlm import run_vlm_equations, run_vlm_variables
    from src_1.pdf_to_images import (
        pdf_to_png_bytes,
    )  # ← ファイル名に合わせて修正
    from src_1.schema_docjson import EQUATION_DB_SCHEMA, VARIABLE_DB_SCHEMA
    ...
    parser.add_argument(
        "--var-out",
        default="DB_variable.json",
        help="output JSON for variables",
    )
    parser.add_argument(
        "--eq-out",
        default="DB_equation.json",
        help="output JSON for equations",
    )
    parser.add_argument(
        "--summary-out",
        type=Path,
    ...
    if args.summary_out:
        write_extraction_summary(
            args.summary_out, var_db, eq_db, args.var_out, args.eq_out
        )
...
def main() -> None:
    ...
    substitutions, remaining_secondary_vars, matched_pairs = (
        harmonize_variables(
    ...
    merged_variables = deduplicate_variables(
        database_vars,
        list(primary_vars_data.get("variables", []))
        + remaining_secondary_vars,
    )
...
def compile_model(
    canon: List[CanonicalEquation], meta: Dict[str, Any]
) -> CompiledModel:
    """
...
# =====================
def run_pipeline(
    doc_id: str, passages: List[Dict[str, Any]], meta: Dict[str, Any]
) -> Dict[str, Any]:
    app = build_graph()
    init: PipelineState = {
        "doc_id": doc_id,
